...
var
  startdate,time : real;
  gameison : boolean;
  quadrant : array[1..8,1..8] of string;
...
procedure insertfeature(a: string; z1,z2: integer);
  begin
    quadrant[z1,z2] := a
  end;

{ See if the sector contains this feature }
function checkfeature(a: string; z1,z2: integer) : boolean;
  begin
    checkfeature := false;
...
procedure readcommand(var cmd:string);
  var
    i: integer;
    line: string;
  begin
    ...
    readln(line);
    i := 1;
    while (i <= 3) and (i <= length(line)) do
      begin
        if line[i] = ' ' then i := 4  { stop }
        else begin
          cmd[i] := line[i];
          i := i + 1
        end
...
{ Read integer coordinate separated by a comma (e.g. "3,5") }
...
function lastletter(st : string) : integer;
  label 100;
  var
    i : integer;
  begin
...
procedure namequadrant(q1,q2: integer; nameonly: boolean; var name: string);
  var
    tmp : string;

  { Append Roman numeral to string }
  procedure appendnum(var name: string; num: string);
    var
      last,i : integer;
    begin
